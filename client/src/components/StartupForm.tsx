...
import { MapPin, Mail, Phone, Users, Building, Tag, FileText, DollarSign, Calendar } from "lucide-react";
...
const StartupForm = () => {
    ...
    const handleSubmit = async () => {
        const {
            name, tags, description, fundingStage, foundedYear, valuationLastRound,
            latitude, longitude, location, contactEmail, contactPhone, founders,
        } = startupDetails;
        if (!name || !tags.length || !description || !fundingStage || !foundedYear ||
            !valuationLastRound || !location || !contactEmail || !contactPhone || !founders) {
        ...
        const newStartup = await createStartup(
            name, tags, description, fundingStage, foundedYear, valuationLastRound,
            location, latitude || 0, longitude || 0, contactEmail, contactPhone, founders, image
        );
        setLoading(false);
        navigate("/home");
        setStartupDetails({
            name: "", tags: [], description: "", fundingStage: "", foundedYear: 2025,
            valuationLastRound: 0, location: "", contactEmail: "", contactPhone: "", founders: "",
        });
    };
        ...
        <>
            ...
            <div className="mb-8">
                <h2 className="text-xl font-semibold mb-3 text-right flex items-center gap-2">
                    <Building className="w-5 h-5" /> תמונת הסטארטאפ
                </h2>
                        ...
                        <Input
                            ...
                            value={
                                startupDetails.valuationLastRound
                                    ? `₪ ${Number(startupDetails.valuationLastRound).toLocaleString("he-IL")}`
                                    : ""
                            }
                            onChange={(e) => {
            ...
            {startupDetails.latitude && startupDetails.longitude && (
                <div className="h-64 mt-6 rounded-lg shadow-md overflow-hidden">
                    ...
                    >
                        ...
                        <Marker position={[startupDetails.latitude, startupDetails.longitude]}>
            ...
            <div className="flex justify-end mt-8">
                ...
                >
                    {loading ? "מעלה את הסטארטאפ..." : "צור סטארטאפ"}
                </Button>
            </div>
        </>
    );
