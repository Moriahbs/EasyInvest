 ...
 */
// CREATE NEW STARTUP
router.post("/", uploadUtils_1.default.single("startupImage"), async (req, res) => {
    try {
        const { tags, foundedYear, valuationLastRound, location, latitude, longitude, name, description, fundingStage, contactEmail, contactPhone, founders, country, } = req.body;
        ...
        const newStartup = new dbUtils_1.Startup({
            tags,
            foundedYear,
            valuationLastRound,
            location,
        ...
        await user.save();
        const found = await dbUtils_1.Startup.findById(newStartup._id).populate("owner");
...
router.get("/sender/:sender", async (req, res) => {
    try {
        ...
        const senderStartups = await dbUtils_1.Startup.find({ owner: sender })
            .populate("owner")
...
router.get("/:id", async (req, res) => {
    try {
        ...
        const found = await dbUtils_1.Startup.findById(id).populate("owner");
        if (!found) {
...
router.put("/:id", uploadUtils_1.default.single("startupImage"), async (req, res) => {
    try {
        ...
        if (tags)
            updatedStartup.tags = tags;
        if (foundedYear)
            updatedStartup.foundedYear = foundedYear;
        if (valuationLastRound)
            updatedStartup.valuationLastRound = valuationLastRound;
        if (latitude)
            updatedStartup.latitude = latitude;
        if (location)
            updatedStartup.location = location;
        if (longitude)
            updatedStartup.longitude = longitude;
        if (name)
            updatedStartup.name = name;
        if (description)
            updatedStartup.description = description;
        if (fundingStage)
            updatedStartup.fundingStage = fundingStage;
        if (contactEmail)
            updatedStartup.contactEmail = contactEmail;
        if (country)
            updatedStartup.country = country;
        if (contactPhone)
            updatedStartup.contactPhone = contactPhone;
        if (founders)
            updatedStartup.founders = founders;
        if (image)
            updatedStartup.image = image;
        const startup = await dbUtils_1.Startup.findById(id).populate("owner");
        const startupToUpdate = startup;
        if (!startupToUpdate) {
            res.status(404).send({ error: "Startup not found" });
            return;
        }
        ...
        const updated = await dbUtils_1.Startup.findOneAndUpdate({ _id: id }, updatedStartup, {
...
router.delete("/:id", async (req, res) => {
    try {
        ...
        const startup = await dbUtils_1.Startup.findById(startupId).populate("owner");
        const startupToDelete = startup;
        if (!startupToDelete) {
            res.status(404).send({ error: "Startup not found" });
            return;
        }
        ...
        await user.save();
        await dbUtils_1.Startup.findByIdAndDelete(startupId);
...
router.post("/visit", async (req, res) => {
    try {
        ...
        const [user, startup] = await Promise.all([
            dbUtils_1.User.findById(userId),
            dbUtils_1.Startup.findById(startupId),
        ]);
        if (!user || !startup) {
        ...
        const updatedStartup = await dbUtils_1.Startup.findByIdAndUpdate(startupId, update, {
            new: true,
        });
...
router.get("/visit/:startupId", async (req, res) => {
    try {
        ...
        const startupObjectId = new mongoose_1.default.Types.ObjectId(startupId);
        let fromDate;
        let groupFormat;
        if (range === "monthly") {
            fromDate = new Date();
        ...
        const result = await dbUtils_1.Startup.aggregate([
